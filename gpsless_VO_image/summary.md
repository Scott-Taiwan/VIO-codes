...
不想每次都真的把 F450 飛起來才能測「連續拍照→VO」這條流程，可以直接手持機身走動測試。**不管哪種供電方式，拆螺旋槳都是必要的安全措施**——`--dry-run` 只是「Python 程式要不要連 Pixhawk」的開關，跟飛控本身有沒有通電是兩回事；連線、讀遙測、送 `GPS_INPUT` 都不會讓飛機解鎖(arm)或動馬達（解鎖需要遙控器/地面站明確下指令，這支程式從頭到尾不會送），但仍要以拆槳為保險。

```bash
# 情境一：只給 Jetson+相機供電（例如行動電源），完全不接飛控。
python3 live_vo_gps.py --handheld --dry-run --interval 1 --save-dir ./handheld_test

# 情境二：整台 F450 通電、飛控跟 Jetson 之間的序列埠有接好，
# 拿真實羅盤航向（甚至 GPS，如果有訊號）當錨點，軌跡方向會更準。
...
- 錨點優先用真實 Pixhawk 遙測（有連線的話）；連不到或缺哪個欄位（例如室內沒有 GPS 定位、但羅盤航向讀得到），缺的部分才補假值 `0`，不會像原本一樣直接報錯退出。看到印出的經緯度是 `0, 0` 附近就代表是補的假值，重點看 `disp=`／`yaw=`／信心標記（OK／LOW-CONF）
